<?php
/**
 * Plugin Name:       WP Admin Mail Tester
 ...
 * Text Domain:       wp-admin-mail-tester
        ...
        if ( wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wpamt_mail_tester_nonce'] ) ), 'wpamt_mail_tester_action' ) ) {
            ...
            if ( empty( $error ) ) {
                if ( wp_mail( $to, $subject, nl2br( $message ), $headers, $attachments ) ) {
                    ...
                    $error = __( 'Failed to send email. Please check your mail server configuration.', 'wp-admin-mail-tester' );
            ...
            $error = __( 'Security check failed. Please try again.', 'wp-admin-mail-tester' );
        }
    }
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('WP Admin Mail Tester', 'wp-admin-mail-tester'); ?></h1>
        <?php if ($sent): ?>
            <div class="notice notice-success"><p><?php esc_html_e('Email sent successfully!', 'wp-admin-mail-tester'); ?></p></div>
        <?php elseif ($error): ?>
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <table class="form-table">
                <tr>
                    <th><label for="wpamt_to"><?php esc_html_e('To Address', 'wp-admin-mail-tester'); ?></label></th>
                ...
                <tr>
                    <th><label for="wpamt_subject"><?php esc_html_e('Subject', 'wp-admin-mail-tester'); ?></label></th>
                    <td><input type="text" name="wpamt_subject" id="wpamt_subject" required value="<?php esc_attr_e('Test Email from WP Admin Mail Tester', 'wp-admin-mail-tester'); ?>" class="regular-text"></td>
                </tr>
                <tr>
                    <th><label for="wpamt_message"><?php esc_html_e('Message', 'wp-admin-mail-tester'); ?></label></th>
                    <td><textarea name="wpamt_message" id="wpamt_message" rows="5" class="large-text"><?php echo esc_textarea( __( 'This is a test email sent from your WordPress site to check mail server functionality.', 'wp-admin-mail-tester' ) ); ?></textarea></td>
                </tr>
                <tr>
                    <th><label for="wpamt_attachment"><?php esc_html_e('Attachment (optional)', 'wp-admin-mail-tester'); ?></label></th>
                    <td><input type="file" name="wpamt_attachment" id="wpamt_attachment"></td>
                </tr>
            </table>
            <p><input type="submit" name="wpamt_send_test_mail" class="button button-primary" value="<?php esc_attr_e('Send Test Email', 'wp-admin-mail-tester'); ?>"></p>
        </form>
    </div>
    <?php
}
